// Add dcustom debris for the vigorWasp
...
////////////////////////////
// Creation of the palicat
////////////////////////////
// Definition of the "palicat" pet
palicat = {
	id:"Palicat",
	item: -1, // must be the index in sc.inventory.items (sc.inventory.getItemID("palikat"))
	autoAdd: true, // Is given to the player automaticly when the game is loaded
	type: "Pet",
	settings: {
		animSheet: "palicat", // The name of the .json file which defines the animation
		walkAnims: {
			idle: "idle",
			move: "move",
			run: "run",
...
class skinHandler {
	constructor(skin) {
		// Check if the item linked has been loaded to the items list
		var skinItemID = sc.inventory.getItemID(skin.id)
		if (skinItemID >= 0) {
			// Change the skin item ID to the correct one
			skin.item = skinItemID
			// Check if the item has already been loaded into the toggle items
			if (!this.isAlreadyInToggleitems(skinItemID)) {
				// We add the items in the toggle sets for pets
				sc.model.player.toggleSets["pet"].items.push(skinItemID.toString());
			}
			// We load the skin into the player skin
			sc.playerSkins.onExtensionLoaded(skin);
		}
	}
	isAlreadyInToggleitems(skinItemID) {
		var toggleItems = sc.model.player.toggleSets["pet"].items;
		for(var i = 0; i < toggleItems.length; i++) {
			if (toggleItems[i] == skinItemID.toString())
				return true;
		}
		return false;
	}
...
ig.module("game.feature.player.entities.palicat-pet").requires("game.feature.npc.entities.sc-actor").defines(function() {
